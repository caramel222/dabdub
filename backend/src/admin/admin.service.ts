...
import { ManualReconciliationDto, ReconciliationType } from './dto/manual-reconciliation.dto';
    ...
    async performManualReconciliation(
        ...
        switch (dto.type) {
            case ReconciliationType.PAYMENT:
                entity = await this.paymentRepo.findOne({ where: { id: dto.entityId } });
                entityType = 'Payment';
                break;
            case ReconciliationType.SETTLEMENT:
                ...
                break;
            default:
                throw new BadRequestException(`Unsupported reconciliation type: ${dto.type}`);
        }
